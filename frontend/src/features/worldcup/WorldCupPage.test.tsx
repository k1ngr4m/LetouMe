...
describe('WorldCupPage odds display', () => {
  ...
  it('opens a full odds modal from compact match cards', async () => {
    ...
    const predictedWinTile = within(dialog).getByRole('group', { name: '主胜 1.80 预测模型 世界杯模型 A' })
    expect(within(predictedWinTile).getByText('世界杯模型 A')).toBeInTheDocument()
    const drawTile = within(dialog).getByRole('group', { name: '平 3.20' })
    expect(within(drawTile).queryByText('世界杯模型 A')).not.toBeInTheDocument()
  ...
  it('groups multi-pick recommendations for a match by play type', async () => {
    ...
    const scoreCardQueries = within(scoreCard as HTMLElement)
    const scoreTwoOneGroup = scoreCardQueries.getByRole('region', { name: '2:1 1 个推荐' })
    const scoreOneNilGroup = scoreCardQueries.getByRole('region', { name: '1:0 1 个推荐' })
    expect(within(scoreTwoOneGroup).getByText('占比 50%')).toBeInTheDocument()
    expect(within(scoreOneNilGroup).getByText('占比 50%')).toBeInTheDocument()
    expect(within(scoreTwoOneGroup).getByText('测试比分推荐 2:1。')).toBeInTheDocument()
    expect(within(scoreOneNilGroup).getByText('测试比分推荐 1:0。')).toBeInTheDocument()
    ...
    const totalGoalsQueries = within(totalGoalsCard as HTMLElement)
    const twoGoalsGroup = totalGoalsQueries.getByRole('region', { name: '2 1 个推荐' })
    const threeGoalsGroup = totalGoalsQueries.getByRole('region', { name: '3 1 个推荐' })
    expect(within(twoGoalsGroup).getByText('占比 50%')).toBeInTheDocument()
    expect(within(threeGoalsGroup).getByText('占比 50%')).toBeInTheDocument()
    expect(within(twoGoalsGroup).getByText('62%')).toBeInTheDocument()
    ...
    const halfFullQueries = within(halfFullCard as HTMLElement)
    const winWinGroup = halfFullQueries.getByRole('region', { name: '胜胜 1 个推荐' })
    const drawDrawGroup = halfFullQueries.getByRole('region', { name: '平平 1 个推荐' })
    expect(within(winWinGroup).getByText('占比 50%')).toBeInTheDocument()
    expect(within(drawDrawGroup).getByText('占比 50%')).toBeInTheDocument()
    expect(within(drawDrawGroup).getByText('54%')).toBeInTheDocument()
  })
  ...
  it('filters recommendations by AI model and keeps model names visible', async () => {
    ...
    const initialWinDrawCard = (await screen.findByRole('heading', { name: '胜平负 · 2 个推荐' })).closest('article')
    expect(initialWinDrawCard).not.toBeNull()
    expect(within(initialWinDrawCard as HTMLElement).getByRole('region', { name: '胜 1 个推荐' })).toBeInTheDocument()
    expect(within(initialWinDrawCard as HTMLElement).getByRole('region', { name: '平 1 个推荐' })).toBeInTheDocument()
    expect(within(initialWinDrawCard as HTMLElement).getAllByText('占比 50%')).toHaveLength(2)

    await userEvent.click(screen.getByRole('button', { name: '世界杯模型 B' }))

    expect(screen.queryByText('测试推荐理由。')).not.toBeInTheDocument()
    expect(screen.getByText('模型 B 认为平局风险收益更均衡。')).toBeInTheDocument()
    const filteredWinDrawCard = screen.getByRole('heading', { name: '胜平负 · 1 个推荐' }).closest('article')
    expect(filteredWinDrawCard).not.toBeNull()
    expect(within(filteredWinDrawCard as HTMLElement).queryByRole('region', { name: '胜 1 个推荐' })).not.toBeInTheDocument()
    const filteredDrawGroup = within(filteredWinDrawCard as HTMLElement).getByRole('region', { name: '平 1 个推荐' })
    expect(within(filteredDrawGroup).getByText('占比 100%')).toBeInTheDocument()
