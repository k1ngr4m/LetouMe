...
class LotteryDataFetcher:
    ...
    def save_to_json(self, data, filename="dlt_data.json", preserve_history=True):
        """
        保存数据到 JSON 文件
        ...
        try:
            if preserve_history:
                ...
                # 同时更新到 ../data/dlt_data.json
                try:
                    web_data_path = os.path.join(os.path.dirname(filename), '..', 'data', 'dlt_data.json')
    ...
    def fetch_and_save(self, output_file="dlt_data.json", preserve_history=True, start=None, end=None):
        """
        获取并保存数据的主函数
