...
class LotteryDataFetcher:
    ...
    def save_to_json(self, data, filename="ssq_data.json", preserve_history=True):
        """
        保存数据到 JSON 文件
        ...
        try:
            if preserve_history:
                ...
                # 同时更新到 ../data/ssq_history.json
                try:
                    web_data_path = os.path.join(os.path.dirname(filename), '..', 'data', 'ssq_history.json')
    ...
    def fetch_and_save(self, output_file="ssq_data.json", preserve_history=True):
        """
        获取并保存数据的主函数
