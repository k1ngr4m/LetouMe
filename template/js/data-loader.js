...
const DataLoader = {
    ...
    async loadLotteryHistory() {
        try {
            const response = await fetch('./data/ssq_history.json');
            if (!response.ok) {
    ...
    async loadPredictions() {
        try {
            const response = await fetch('./data/ssq_ai_predictions.json');
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`);
            }
            const data = await response.json();
    ...
    async loadPredictionsHistory() {
        try {
            const response = await fetch('./data/ssq_predictions_history.json');
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`);
            }
            const data = await response.json();
