...
class MyBetService:
    ...
    def list_records(
        ...
        *,
        ...
        filters = {
            ...
            "ticket_time_value": ticket_time_value,
            "ticket_time_start": ticket_time_start,
            "ticket_time_end": ticket_time_end,
            "ticket_time_operator": str(ticket_time_operator or "eq").strip().lower(),
            "ticket_time_dynamic": str(ticket_time_dynamic or "").strip().lower(),
            "ticket_time_dynamic_start": ticket_time_dynamic_start,
            "ticket_time_dynamic_end": ticket_time_dynamic_end,
            "created_time_value": created_time_value,
            "created_time_start": created_time_start,
            "created_time_end": created_time_end,
            "created_time_operator": str(created_time_operator or "eq").strip().lower(),
            "created_time_dynamic": str(created_time_dynamic or "").strip().lower(),
    ...
    def _build_pl3_line_payload(self, payload: dict[str, Any], *, multiplier: int) -> dict[str, Any]:
        ...
        if play_type == "pl3_compound":
            ...
            positions = [self._normalize_numbers(values, valid_range=self.DIGIT_RANGE) for values in raw_positions[:3]]
            if any(not values for values in positions):
                raise ValueError("直选定位复式每位至少选择 1 个号码")
            bet_count = len(positions[0]) * len(positions[1]) * len(positions[2])
            return {
    ...
    def _calculate_pl3_line_settlement(self, line: dict[str, Any], draw: dict[str, Any]) -> dict[str, Any]:
        ...
        if play_type == "direct":
            ...
            if matched:
                level = "直选"
                winning_count = 1
        elif play_type in {"pl3_dantuo", "pl3_compound"}:
            hundreds = list(line.get("direct_hundreds") or [])
            tens = list(line.get("direct_tens") or [])
            units = list(line.get("direct_units") or [])
            matched = len(digits) == 3 and digits[0] in hundreds and digits[1] in tens and digits[2] in units
